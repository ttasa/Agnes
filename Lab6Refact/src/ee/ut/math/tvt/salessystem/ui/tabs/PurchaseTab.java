...
public class PurchaseTab {
	...
	protected void newPurchaseButtonClicked() {
		...
		Client client = selectClientFromDialog();
		model.getSaleController().startNewSale(client);
		startNewSale();
	}
	...
	protected void startPayingPurchase() {
		double price = model.getSaleController().getSale().getSum();
	...
	public void endPurchaseAfterPaying() throws VerificationFailedException {
		...
		model.getDomainController().registerSale(
				model.getSaleController().getSale());
		endSale();
	}
	...
	private void refresh() {
		model.getSaleController().endSale();
		model.getCurrentPurchaseTableModel().fireTableDataChanged();
	}

	private Client selectClientFromDialog() {
