...
public class ConsoleUI {
	...
	private void processCommand(String command) {
		...
		if (c[0].equals("h")) {
			...
			try {
				sc.startNewSale(selectedClient);
				for (StockItem stockItem : cart) {
					sc.addItemToSale(new SoldItem(stockItem, stockItem
							.getQuantity()));
				}
				dc.registerSale(sc.getSale());
				sc.endSale();
				cart.clear();
			} catch (Exception e) {
				log.error(e.getMessage());
